import json
import time

import requests

cookies = {
    'entryChannel': '22',
    'entryUrl': 'https%3A%2F%2Fwww.trivago.com.tr%2F',
    'userLanguage': 'en-US',
    'edge_tid_s': 'd36bcdfb7abed25ff5d23da202',
    'edge_tid': 'd36bcdfb7abed25ff5d23da202',
    'trv_wbs': '1240',
    'trv_wb': '1240',
    'ak_bmsc': 'DEBB0D84453DF78C6A5FE1FF23B2E5A1~000000000000000000000000000000~',
    'DT_SEL': '%7B%22source%22%3A0%2C%22detail%22%3A0%7D',
    'RC_AC': '1',
    'trv_user_selection': 'rc-1-2',
}

headers = {
    'accept': 'application/graphql-response+json, application/graphql+json, application/json, text/event-stream, multipart/mixed',
    'accept-language': 'en-US,en;q=0.9,tr;q=0.8',
    'apollographql-client-name': 'hs-web-app',
    'apollographql-client-version': '583b5ed5',
    'content-type': 'application/json',
    'ect': '4g',
    'origin': 'https://www.trivago.com.tr',
    'priority': 'u=1, i',
    'referer': 'https://www.trivago.com.tr/en-US/srl/hotels-antalya-t%C3%BCrkiye?search=200-15247;dr-20250730-20250731;rc-1-2',
    'sec-ch-ua': '"Not)A;Brand";v="8", "Chromium";v="138", "Microsoft Edge";v="138"',
    'sec-ch-ua-mobile': '?0',
    'sec-ch-ua-platform': '"Windows"',
    'sec-fetch-dest': 'empty',
    'sec-fetch-mode': 'cors',
    'sec-fetch-site': 'same-origin',
    'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/138.0.0.0 Safari/537.36 Edg/138.0.0.0',
    'x-trv-app-id': 'HS_WEB_APP_WARP',
    'x-trv-connection-id': 'qT9m0aLurCD866FZzaAoIxdvJq1',
    'x-trv-cst': '55622,46164,48405,51886,52345,56034,70018-2,70089,70246,70290,70332,70407,70579,70620-2,70623,70773-2,70785,70766,70826,70830,70928-1,70886,70806,70739-2,70511,70916,71028-1,70798,71023,71142,70841,71057-2,70819,71118-1,71230,71254,71263,71285,71292,70839,71194,71393-1,71172,71475,71469,71476,71289,71447,71460,71431,71428,71549,71550,71553-1,71554-1,71512,71498,71511,71523,71173,71569,71598,71599,71597,71633,71591,71619,71586,71402,71615,71572-1,71410,71561,71479-1,71315,71590,71734,71594,71576,71583,71560-3,71621,71654,71725-2,71700,71646,71704-2,71707,71759-1,71728,71691,71770,71581,71605-2,71029,71693-2,71726-2,71663,71680-2,71613,71716,71721,71650-1,71805,71773,71750,71699-2,71748,71743,71712,71744,71610,71715-1,71796-3,71060-1,71788,71683',
    'x-trv-currency': 'TRY',
    'x-trv-language': 'en-US',
    'x-trv-platform': 'tr',
    'x-trv-sequence-id': '124',
    'x-trv-tid': 'd36bcdfb7abed25ff5d23da202',
    # 'cookie': 'entryChannel=22; entryUrl=https%3A%2F%2Fwww.trivago.com.tr%2F; userLanguage=en-US; edge_tid_s=d36bcdfb7abed25ff5d23da202; edge_tid=d36bcdfb7abed25ff5d23da202; trv_wbs=1240; trv_wb=1240; ak_bmsc=DEBB0D84453DF78C6A5FE1FF23B2E5A1~000000000000000000000000000000~; DT_SEL=%7B%22source%22%3A0%2C%22detail%22%3A0%7D; RC_AC=1; trv_user_selection=rc-1-2',
}

json_data = {
    'extensions': {
        'persistedQuery': {
            'sha256Hash': 'a1ff0db1525c5c5313a494acb86c5823253f9d8983676baeeb15e3e8566783da',
            'version': 1,
        },
    },
    'operationName': 'infoSlideoutQuery',
    'variables': {
        'input': {
            'nsids': [
                {
                    'id': 28348832,
                    'ns': 100,
                },
            ],
        },
        'shouldIncludeClosestConcept': False,
    },
}




def fetch_hotel_info(ns_id, ns_ns):

    json_data['variables']['input']['nsids'][0]['id'] = ns_id
    json_data['variables']['input']['nsids'][0]['ns'] = ns_ns
    
    response = requests.post('https://www.trivago.com.tr/graphql?infoSlideoutQuery', cookies=cookies, headers=headers, json=json_data)
    response_json = response.json()

    hotel_email = response_json['data']['getAccommodationDetails']['accommodationDetails'][0]['contactDetails']['email']
    hotel_phone = response_json['data']['getAccommodationDetails']['accommodationDetails'][0]['contactDetails']['phone']
    hotel_website = response_json['data']['getAccommodationDetails']['accommodationDetails'][0]['contactDetails']['website']
    
    return hotel_email, hotel_phone, hotel_website





processed_hotels = []
with open('hotels_data.json', 'r', encoding='utf-8') as f:
    hotels = json.load(f)
    
    for group in hotels:
        accommodations_list = group.get('data', {}).get('accommodationSearchResponse', {}).get('accommodations', [])
        for accommodation in accommodations_list:
            try:
                
                accommodation_details = accommodation.get('accommodationDetails', {})
                if not accommodation_details:
                    continue
                # translatedName -> value
                translated_name = accommodation_details.get('translatedName', {}).get('value', '')
                if not translated_name:
                    continue
                hotel_classification = accommodation_details.get('hotelClassification', {})
                if not hotel_classification:
                    hotel_classification = 'N/A'
                else:
                    hotel_classification = hotel_classification.get('rating', '')
                
                if not hotel_classification:
                    hotel_classification = 'N/A'
                nsid = accommodation_details.get('nsid', {})
                if not nsid:
                    continue
                if not nsid.get('id') or not nsid.get('ns'):
                    continue
                ns_id = accommodation_details.get('nsid').get('id')
                ns_ns = accommodation_details.get('nsid').get('ns')
                hotel_email, hotel_phone, hotel_website = fetch_hotel_info(ns_id, ns_ns)
                print(translated_name, hotel_classification, hotel_email, hotel_phone, hotel_website)
                hotel = {
                    'name': translated_name,
                    'classification': hotel_classification,
                    'email': hotel_email,
                    'phone': hotel_phone,
                    'website': hotel_website
                    }
                processed_hotels.append(hotel)
                print('Waiting 100ms before next request...')
            except Exception as e:
                print(f"Error processing hotel: {e}")
                continue

with open('processed_hotels.json', 'w', encoding='utf-8') as f:
    json.dump(processed_hotels, f, ensure_ascii=False, indent=4)