...
request_responses=[]
while True:
    json_data['variables']['params']['offset'] = offset
    ...
    if response.status_code != 200:
        ...
        break
    data = response.json()
    # check if data['data']['accommodationSearchResponse']['accommodations'] is empty array
    if not data.get('data', {}).get('accommodationSearchResponse', {}).get('accommodations', []):
        print("No more accommodations found, stopping the loop.")
        break
    request_responses.append(data)
...
with open('hotels_data.json', 'w', encoding='utf-8') as f:
    import json
    json.dump(request_responses, f, ensure_ascii=False, indent=4)